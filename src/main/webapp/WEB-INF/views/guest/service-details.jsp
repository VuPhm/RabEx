...
<head>
    ...
    <title>Dịch vụ ${service.name}</title>
</head>
<body>
<%-- Nav --%>
...
<!-- DELIVERY Service Detail Begin -->
<c:if test="${service.type == 'DELIVERY'}">
...
<!-- DELIVERY Service Detail End -->


<!-- ADD-ON Service Detail Begin -->
<c:if test="${service.type == 'ADD_ON'}">
    <div class="container mb3">
        <div class="h2 text-uppercase mb-3">Thông tin Dịch vụ ${service.name}</div>
        <div class="h3 text-primary text-uppercase">I. ĐỊNH NGHĨA</div>
        <p>
        ...
        <div class="table-responsive">
            <table class="table table-bordered mt-3 text-center" style="table-layout: auto;">
                ...
                <tbody>
                    <%--@elvariable id="tiers" type="java.util.List"--%>
                    <%--@elvariable id="tier" type="com.rabex.express.model.SurchargeTier"--%>
                <c:forEach var="tier" items="${tiers}">
                    <tr>
                        <td>${tier.description}</td>
                        <td>mô tả: ${tier.description}</td>
                        <td>${tier.displayInfo()}</td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
